...
The cl-plplot-system package has been changed to only export the "wrapped"
versions of the PLplot functions.
...
that contains the interface to the large plplot API, and the other is a 
(hopefully) easy to use front end (cl-plplot). It should all be Common Lisp 
& ASDF installable, but let me know if you find otherwise.
...
2) types.lisp defines the CFFI types that are used to wrap the PLplot library.
3) pl-defcfun.lisp defines the macro that is used to facilitate wrapping all 
   the CFFI calls.
4) misc.lisp contains some helper functions and macros.
5) api.lisp contains all the plplot library / CFFI interface calls. Not
